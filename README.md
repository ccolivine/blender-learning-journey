...
## Day 14 - 人物建模进阶：UV展开、贴图绘制与头发制作 (2026-08-17)

### 今日状态
- **学习进度**：完成教程第2、3、4期（UV展开、贴图绘制、头发建模）。
- **核心收获**：首次完整实践人物模型的UV拆解、贴图绘制与毛发制作流程。
- **前置优势**：SAI2绘画经验在贴图绘制阶段直接平移生效。

### 今日目标
- 掌握人物头部的UV展开方法（标记缝合边 → 展开 → 布局调整）
- 完成头部的贴图绘制（肤色、五官、阴影）
- 完成头发的基础建模（发片制作、梳理摆放）
- 将绘画经验迁移到3D贴图绘制中
...
![UV展开与贴图绘制](images/day14_uv_texture.jpg)

![头发建模正视图](images/day14_hair_modeling1.jpg)

![头发建模侧视图](images/day14_hair_modeling2.jpg)

### 学习心得与技术要点

**1. UV展开：“剥橘子皮”的3D版**

第一次完整做UV展开，之前学场景时虽然接触过，但那时候贴图简单，没有像今天这样精细拆解。核心收获是：

- **缝合边标记**：就像拆纸箱一样，先想好“从哪里剪开”再动手，而不是边剪边想。标记缝合边决定了UV展开的平整度和拉伸程度。
- **展开后的布局调整**：让UV岛尽量铺满0-1空间，减少浪费，同时让接缝藏在不显眼的位置（如耳后、头发遮挡处）。
- **一个经验**：脸部的UV尽量少切，大块展开比切成碎片更容易画贴图。

**2. 贴图绘制 = SAI2 + 3D预览的联合作战**

今天画贴图的流程是：
- 在Blender里展开UV → 导出UV布局图(.png)
- 在SAI2中打开UV布局图作为参考层，在上面画肤色、五官、阴影
- 保存后导回Blender，连接到材质的“基础色”
- 在材质预览模式下实时检查，再回到SAI2修改

**优势明显**：用SAI2的笔刷画3D贴图，比直接在Blender的纹理绘制模式里画顺手很多，因为SAI2的图层、笔刷、混色逻辑我已经有肌肉记忆了。

**缺点**：修改一次就要导出→导入→刷新材质，迭代速度不如直接在Blender里画快。所以以后简单的颜色用Blender画，精细的细节用SAI2画。

**3. 头发建模：从“一块板”到“有层次”**

头发是三渲二风格里最重要的视觉元素之一。今天掌握的流程：
- **发片制作**：从平面开始，挤出厚度 → 用贝塞尔曲线或手动调整弧度 → 形成一片头发
- **梳理摆放**：先确定刘海走向，再到侧发、后发，逐层叠加
- **关键技巧**：用“平滑”和“细分”修改器让发片有自然的弧度，同时保持足够的面数让边缘不毛躁

### 明日计划
- 继续教程后续内容：身体建模、服装、面部五官细化
- 将今日制作的贴图与模型整合进游戏demo测试

### 今日资源
- 跟学视频：BV17P4y1v7ct（QQ人建模教程 · 第2-4期）
- 学习模式：功能掌握 + 实操并行

## Day 15 - 人物建模继续：身体框架与阶段性策略调整 (2026-08-18)

### 今日状态
- **学习进度**：完成教程第6节前半部分（身体基础框架搭建），骨骼部分计划延后。
- **技术决策**：跳过“抢夺材质”（材质烘焙）环节——因模型后续需导入游戏引擎单独烘焙，现阶段不做重复工作。
- **学习状态**：保持每日推进，但根据实际需求灵活裁剪教程内容。

### 今日目标
- 学习人体基础比例与躯干建模方法
- 完成躯干的基础形状搭建（胸腔→腰部→胯部）
- 理解四肢的建模逻辑（手臂、腿部轮廓）

### 过程记录

![身体框架搭建中](images/day15_body_blockout.jpg)

### 学习心得与技术要点

**1. “半节课”的进度是合理的**

今天教程看到第6节，内容是身体基础框架（躯干+四肢轮廓）。骨骼部分还没有展开，刚好卡在一个自然的暂停点。

这种“听一半”的状态其实是好的——身体的结构逻辑已经建立起来了，剩下的骨骼绑定是独立模块，完全可以等模型基础定型后再补。

**2. 跳过“抢夺材质”的决策逻辑**

教程里有一个环节是把材质从高模“抢”到低模上（烘焙法线贴图）。我选择跳过，原因是：

- **最终目标是导入游戏引擎**：游戏引擎（如Unity/Godot）中的材质和光照系统与Blender不同，在Blender里烘焙的贴图到引擎里往往需要重新调整
- **流程简化**：现阶段先完成模型本身，材质烘焙留到引擎里做更直接
- **时间效率**：与其花时间在Blender里做一套“可能用不上”的烘焙流程，不如先推进模型进度

### 明日计划
- 完成第6节后半部分（身体细化）
- 评估是否需要进入骨骼绑定，或先暂停人物方向、回头优化已有场景

### 今日资源
- 跟学视频：BV17P4y1v7ct（QQ人建模教程 · 第6节前半）
- 学习模式：目标导向，灵活裁剪

## Day 16 - 骨骼绑定与身体UV展开 (2026-08-19)

### 今日状态
- **学习进度**：完成骨骼绑定基础操作，同步完成身体部分的UV展开。
- **核心动作**：让角色从“静态模型”变为“可动姿态”，为后续导入游戏引擎做准备。
- **技术决策**：骨骼绑定以基础动作为目标（非精细面部绑定），满足游戏内展示需求即可。

### 今日目标
- 完成骨骼系统的添加与对齐（躯干 → 四肢 → 头部）
- 掌握权重绘制的基础方法（让网格跟随骨骼运动）
- 完成身体部分的UV展开（躯干+四肢）
- 确认模型已具备导入游戏引擎的基础条件

### 过程记录

![骨骼绑定结果](images/day16_rigging.jpg)

![身体UV展开](images/body.jpg)

### 学习心得与技术要点

**1. 骨骼绑定：从“不会动”到“会动”**

人物建模完成后的下一步就是让它“活”起来：
- **骨骼添加**：`Shift+A` → 骨架 → 选择适合人体的骨骼类型（如 `Meta-Rig` 或基础骨架）
- **骨骼对齐**：在编辑模式下调整骨骼位置，使其与模型的身体结构匹配（肩关节、肘关节、膝关节等关键点）
- **权重绘制**：让网格顶点跟随对应骨骼运动——这是最需要耐心的环节

**2. 权重绘制：今天最“磨人”的部分**

权重绘制的本质是告诉每个顶点“你受哪根骨骼影响、影响程度是多少”。今天在权重绘制上花了比较多时间，原因是：
- **影响范围控制**：肩关节附近容易“拉扯”到不该动的地方（比如肩膀动了但手臂没完全跟上，或者胸部被肩骨扯变形）
- **自动权重 vs 手动调整**：先用了“自动权重”让Blender初算一遍，再手动修正明显错误的地方

画权重画得比较细，因为要在游戏里动，穿模会很难看。虽然还不太会画，但至少知道怎么进权重绘制模式、怎么刷了。

**3. 身体UV展开的考量**

给身体展UV和之前做头部时类似，但这次考虑到后续可能会在游戏里换装或改色，UV的布局特意做得规整了一些：
- **躯干作为一个整体**，尽量少切
- **四肢分开展开**，方便单独调整纹理
- **接缝位置**：藏在身体侧面、腋下、大腿内侧等不显眼的位置

**4. 关于“游戏引擎导入”的前置准备**

今天在绑定骨骼时，一直在想一个问题：Blender里绑好的骨骼，到Unity/Godot里能直接用吗？查了一下大概方向：
- 导出FBX格式时需勾选“骨骼”和“形变”
- Unity里需要做重定向（如果用的是人形骨架）或保持通用骨架
- 引擎里会有自己的动画系统，但不影响Blender里绑定好的骨骼结构

目前确认做到“骨骼能驱动网格”这一步就够用，具体的引擎适配留到导入时再调试。

### 当前人物建模进度

| 部位 | 完成度 | 备注 |
| :--- | :--- | :--- |
| 头部 | 已完成 | UV展开+贴图绘制+头发已做 |
| 躯干 | 已完成 | 已展UV |
| 四肢 | 已完成 | 已展UV |
| 骨骼绑定 | 基础完成 | 权重需进一步微调 |
| 材质烘焙 | 跳过 | 引擎内单独处理 |
